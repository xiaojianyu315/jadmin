...
public class DateUtil {
	 ...
	 * 获取当天的指定时间字符串
	 * add by guanganggang
	 * createTime 2016-4-27下午4:31:32
	 * @param date 当前时间
	 * @param time 当天指定时间的时分秒
	 * @return
	 */
	public static String getStrTime(Date date, String time) {
		if (date == null) {
			return "";
		} else {
...
*/	public static long daysDiff(Date startDate, Date endDate){
	 ...
	 * @return
	 */
	public static Date addDay(Date dInput, int day) {
		if (dInput == null) {
			return null;
		}
		Calendar c = Calendar.getInstance();
...
//        Calendar shi8 = Calendar.getInstance();
//        shi8.set(Calendar.MONTH,Calendar.OCTOBER);
//        shi8.set(Calendar.DAY_OF_MONTH,8);
//
//        Calendar shi9 = Calendar.getInstance();
//        shi9.set(Calendar.MONTH,Calendar.OCTOBER);
//        shi9.set(Calendar.DAY_OF_MONTH,9);
        
//        String currentDate = dfs.format(src.getTime());
//        String shi8Date = dfs.format(shi8.getTime());
//        String shi9Date = dfs.format(shi9.getTime());
        
        // 十月8和9是周六日，但由于国庆倒休为工作日，特做处理 2016-09-28 下午 19：55 haihao
//        if(currentDate.equals(shi8Date) || currentDate.equals(shi9Date)){
//        	return false;
//        }
        
        //先检查是否是周六周日(有些国家是周五周六)
        if(src.get(Calendar.MONTH) == 0 && src.get(Calendar.DAY_OF_MONTH) == 4)
        	return false;
     ...
     * @author  shenjunjie
     * 
     * update by zhangjie 2016.2.2 17:30
     * 修复代码 假日判断逻辑对象问题
     * 
     */
    private static void initHolidayList()
    {
        ...
        //元旦
        Calendar yuan1 = Calendar.getInstance();
        yuan1.set(Calendar.MONTH,Calendar.DECEMBER);
        yuan1.set(Calendar.DAY_OF_MONTH, 31);//十二月31日
        holidayList.add(yuan1);
        Calendar yuan2 = Calendar.getInstance();
        yuan2.set(Calendar.MONTH,Calendar.JANUARY);
        yuan2.set(Calendar.DAY_OF_MONTH, 1);//一月1日
        holidayList.add(yuan2);
        Calendar yuan3 = Calendar.getInstance();
        yuan3.set(Calendar.MONTH,Calendar.JANUARY);
        yuan3.set(Calendar.DAY_OF_MONTH, 2);//一月2日
        holidayList.add(yuan3);
        
        //春节
        Calendar springFestival1 = Calendar.getInstance();
        springFestival1.set(Calendar.MONTH,Calendar.JANUARY);
        springFestival1.set(Calendar.DAY_OF_MONTH,27); // 一月27日
        holidayList.add(springFestival1);
        
        Calendar springFestival2 = Calendar.getInstance();
        springFestival2.set(Calendar.MONTH,Calendar.JANUARY);
        springFestival2.set(Calendar.DAY_OF_MONTH,28); // 一月28日
        holidayList.add(springFestival2);
        
        Calendar springFestival3 = Calendar.getInstance();
        springFestival3.set(Calendar.MONTH,Calendar.JANUARY);
        springFestival3.set(Calendar.DAY_OF_MONTH,29); // 一月29日
        holidayList.add(springFestival3);
        
        Calendar springFestival4 = Calendar.getInstance();
        springFestival4.set(Calendar.MONTH,Calendar.JANUARY);
        springFestival4.set(Calendar.DAY_OF_MONTH,30); // 一月30日
        holidayList.add(springFestival4);
        
        Calendar springFestival5 = Calendar.getInstance();
        springFestival5.set(Calendar.MONTH,Calendar.JANUARY);
        springFestival5.set(Calendar.DAY_OF_MONTH,31); // 一月31日
        holidayList.add(springFestival5);
        
        Calendar springFestival6 = Calendar.getInstance();
        springFestival6.set(Calendar.MONTH,Calendar.FEBRUARY);
        springFestival6.set(Calendar.DAY_OF_MONTH,1); // 二月1日
        holidayList.add(springFestival6);
        
        Calendar springFestival7 = Calendar.getInstance();
        springFestival7.set(Calendar.MONTH,Calendar.FEBRUARY);
        springFestival7.set(Calendar.DAY_OF_MONTH,2); // 二月2日
        holidayList.add(springFestival7);
        
        //清明
        Calendar qing1 = Calendar.getInstance();
		qing1.set(Calendar.MONTH,Calendar.APRIL);
		qing1.set(Calendar.DAY_OF_MONTH,2); // 四月2日
        holidayList.add(qing1);

		Calendar qing2 = Calendar.getInstance();
		qing2.set(Calendar.MONTH,Calendar.APRIL);
		qing2.set(Calendar.DAY_OF_MONTH,3); // 四月3日
		holidayList.add(qing2);

		Calendar qing3 = Calendar.getInstance();
		qing3.set(Calendar.MONTH,Calendar.APRIL);
		qing3.set(Calendar.DAY_OF_MONTH,4); // 四月4日
		holidayList.add(qing3);
        
        //五一劳动节
        Calendar may1 = Calendar.getInstance();
        may1.set(Calendar.MONTH,Calendar.APRIL);
        may1.set(Calendar.DAY_OF_MONTH,29); // 四月29日
        holidayList.add(may1);
        
        Calendar may2 = Calendar.getInstance();
        may2.set(Calendar.MONTH,Calendar.APRIL);
        may2.set(Calendar.DAY_OF_MONTH,30); // 四月30日
        holidayList.add(may2);
 
        Calendar may3 = Calendar.getInstance();
        may3.set(Calendar.MONTH,Calendar.MAY);
        may3.set(Calendar.DAY_OF_MONTH,1); // 五月1日
        holidayList.add(may3);
 
        //端午
        Calendar duan = Calendar.getInstance();
        duan.set(Calendar.MONTH,Calendar.MAY);
        duan.set(Calendar.DAY_OF_MONTH,28); // 五月28日
        holidayList.add(duan);
        
        Calendar duan2 = Calendar.getInstance();
        duan2.set(Calendar.MONTH,Calendar.MAY);
        duan2.set(Calendar.DAY_OF_MONTH,29); // 五月29日
        holidayList.add(duan2);
        
        Calendar duan3 = Calendar.getInstance();
        duan3.set(Calendar.MONTH,Calendar.MAY);
        duan3.set(Calendar.DAY_OF_MONTH,30); // 五月30日
        holidayList.add(duan3);
        
        //国庆与中秋
        Calendar shi1 = Calendar.getInstance();
        shi1.set(Calendar.MONTH,Calendar.OCTOBER);
        shi1.set(Calendar.DAY_OF_MONTH,1); // 十月1日
        holidayList.add(shi1);
        
        Calendar shi2 = Calendar.getInstance();
        shi2.set(Calendar.MONTH,Calendar.OCTOBER);
        shi2.set(Calendar.DAY_OF_MONTH,2); // 十月2日
        holidayList.add(shi2);
        
        Calendar shi3 = Calendar.getInstance();
        shi3.set(Calendar.MONTH,Calendar.OCTOBER);
        shi3.set(Calendar.DAY_OF_MONTH,3); // 十月3日
        holidayList.add(shi3);
        
        Calendar zhongQiu = Calendar.getInstance();
		zhongQiu.set(Calendar.MONTH,Calendar.OCTOBER);
		zhongQiu.set(Calendar.DAY_OF_MONTH,4); // 十月4日
        holidayList.add(zhongQiu);
        
        Calendar shi5 = Calendar.getInstance();
        shi5.set(Calendar.MONTH,Calendar.OCTOBER);
        shi5.set(Calendar.DAY_OF_MONTH,5); // 十月5日
        holidayList.add(shi5);
        
        Calendar shi6 = Calendar.getInstance();
        shi6.set(Calendar.MONTH,Calendar.OCTOBER);
        shi6.set(Calendar.DAY_OF_MONTH,6); // 十月6日
        holidayList.add(shi6);
        
        Calendar shi7 = Calendar.getInstance();
        shi7.set(Calendar.MONTH,Calendar.OCTOBER);
        shi7.set(Calendar.DAY_OF_MONTH,7); // 十月7日
        holidayList.add(shi7);

		Calendar shi8 = Calendar.getInstance();
		shi8.set(Calendar.MONTH,Calendar.OCTOBER);
		shi8.set(Calendar.DAY_OF_MONTH,8); // 十月8日
		holidayList.add(shi8);
    }
    /**
     * 
     * @Title: getMonthSpace
     * @Description:得到月份差
     * @author:sunwei
     * @createTime:2016年4月10日下午1:37:31
     * @param startTime开始时间
     * @param endTime 到期时间
     * @return 相差月数
     */
    ...
    public static void main(String[] arg){
    	Date startDate = DateUtil.string2Date("2016-07-25");
    	Date endDate = DateUtil.string2Date("2016-07-26");
    	System.out.println(daysDiff2(startDate, endDate));
	...
	/**
     * 描述：判断当前日期时间是否在一个日期区间内
     * haihao 2016-05-10 下午 14:30
     * @param startTime
     * @param endTime
     * @return
     */
    public static boolean dateIsInterval(String startTime, String endTime){
    	
		boolean flag = false;
		try {
			SimpleDateFormat simpleFormat = new SimpleDateFormat(DateUtil.YYYY_MM_DD_HH_MM_SS);
			Date startDate = simpleFormat.parse(startTime);
			Date endDate = simpleFormat.parse(endTime);
			flag = dateIsInterval(startDate, endDate);
		} catch (ParseException e) {
			System.out.println("Method >>>>> dateIsInterval() 异常，请看以下堆栈信息！");
			e.printStackTrace();
			return flag;
		}
		return flag;
     ...
     * @return
     */
    public static boolean dateIsInterval(Date startTime, Date endTime){
		boolean flag = false;
	 ...
	 *  
	 * @param d1 
	 * @param str 字符串类型日期“2016-07-13 00:00:00”
	 * @return 前者大于后者返回true 反之false 
	 */  
	public static boolean compareDate(Date d1, String str) {
	    Calendar c1 = Calendar.getInstance();  
	    Calendar c2 = Calendar.getInstance();  
	    c1.setTime(d1);  
	    c2.setTime(DateUtil.string2Date(str));  
	    int result = c1.compareTo(c2);  
	    if (result >= 0)  
	        return true;  
	    else  
	...
	/**
     * 目前只支持yyyy-MM-dd HH:mm:ss和yyyy-MM-dd 两种形式的日期转换
     *
     * @param s
     * @param dateType yyyy-MM-dd 或 yyyy-10-20 或 yyyy-MM-dd HH:mm:ss 或 yyyy-MM-dd 23:59:ss
     * @return
     * @throws ParseException
     */
	public static Date getDate2Date(Date s, String dateType) throws ParseException {
		SimpleDateFormat dateFormat = new SimpleDateFormat(dateType);
		String dateStr = dateFormat.format(s);
